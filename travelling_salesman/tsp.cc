...
long double calbound (route t, double** distance){
	...
	for(int i = 0; i < novisits; i++){
		...
		if(current < minToGoal)
			minToGoal = current;
		m1 = distance[t.novisit[i]][t.path.back()];
		m2 = numeric_limits<long double>::max();
...
void tsp( vector< pair<double,double> >& vets ){
	...
	while( !rpq.empty() ){
		...
		novisitsize = current.novisit.size();
        	cout<<"current priority queue size "<<rpq.size()<<" unvisited cities in current path "<<novisitsize<<endl;
		for(int i = 0; i < novisitsize; i++){
		 	next = current;
		 	next.path.push_back(current.novisit[i]);
		 	next.novisit.erase(next.novisit.begin()+i);
		 	next.cost = current.cost + distance[current.novisit[i]][current.path.back()];
			if (novisitsize==2){
