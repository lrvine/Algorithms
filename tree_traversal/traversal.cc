...
char listnode::get_data() { return data; }
...
listnode* listnode::get_left_node() { return left; }

listnode* listnode::get_right_node() { return right; }

void handle_node(listnode* node) { cout << node->get_data() << " "; }
  ...
  while (!ss.empty() || (current != NULL)) {
    if (current != NULL) {
      ...
      if ((current->get_right_node() != NULL) &&
          (current->get_right_node() != lastvisited)) {
        current = current->get_right_node();
      } else {
        handle_node(current);
        lastvisited = current;
